class DataController < ApplicationController
	...
	def trips
		...
		Trip.where("route_id = ? AND direction_id = ?", @route.route_id, @direction_id).order("service_id, trip_id").each do |t|
			...
			if(t.service_id == "7" || t.service_id == 'S3')
				@sundays << trip
			elsif(t.service_id == "5" || t.service_id == 'S2')
				@saturdays << trip
			else
				@weekdays << trip
			end
	...
	def trip_service_label (trip)
		label = "Weekday"
		if(trip.service_id == "7"|| trip.service_id == 'S3')
			label = "Sunday"
		elsif(trip.service_id == "5"|| trip.service_id == 'S2')
			label = "Saturday"
		end
		label
	end
